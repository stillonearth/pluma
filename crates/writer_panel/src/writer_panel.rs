...
use writer_panel_settings::{ShowIndentGuides, WriterPanelDockPosition, WriterPanelSettings};

actions!(
    writer_panel,
    [
        /// Collapses all entries in the writer tree.
        CollapseAllEntries,
        /// Collapses the currently selected entry.
        CollapseSelectedEntry,
        /// Expands all entries in the writer tree.
        ExpandAllEntries,
...
pub struct WriterPanel {
    ...
    writer_fetch_tasks: HashMap<(BufferId, ExcerptId), Task<()>>,
...
impl SearchState {
    fn new(
        ...
        Self {
            ...
            _search_match_notify: cx.spawn_in(window, async move |writer_panel, cx| {
                loop {
                    ...
                    let update_result = writer_panel.update(cx, |_, cx| {
                        cx.notify();
                    });
...
struct Excerpt {
    ...
    writers: ExcerptOutlines,
}

impl Excerpt {
    fn invalidate_writers(&mut self) {
        if let ExcerptOutlines::Outlines(valid_writers) = &mut self.writers {
            self.writers = ExcerptOutlines::Invalidated(std::mem::take(valid_writers));
        }
    }

    fn iter_writers(&self) -> impl Iterator<Item = &Outline> {
        match &self.writers {
            ExcerptOutlines::Outlines(writers) => writers.iter(),
            ExcerptOutlines::Invalidated(writers) => writers.iter(),
            ExcerptOutlines::NotFetched => [].iter(),
        }
    }

    fn should_fetch_writers(&self) -> bool {
        match &self.writers {
...
impl PartialEq for OutlineEntryOutline {
    fn eq(&self, other: &Self) -> bool {
        self.buffer_id == other.buffer_id
            ...
            && self.writer.depth == other.writer.depth
            && self.writer.range == other.writer.range
            && self.writer.text == other.writer.text
    }
}
...
impl OutlineEntry {
    fn ids(&self) -> (BufferId, ExcerptId) {
        match self {
            ...
            OutlineEntry::Outline(writer) => (writer.buffer_id, writer.excerpt_id),
        }
    }
}
...
struct SerializedWriterPanel {
    width: Option<Pixels>,
    active: Option<bool>,
}

pub fn init_settings(cx: &mut App) {
    WriterPanelSettings::register(cx);
}
...
pub fn init(cx: &mut App) {
    ...
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        workspace.register_action(|workspace, _: &ToggleFocus, window, cx| {
            workspace.toggle_panel_focus::<WriterPanel>(window, cx);
        });
    })
    .detach();
}
...
impl WriterPanel {
    pub async fn load(
        ...
        let serialized_panel = match workspace
            .read_with(&cx, |workspace, _| {
                WriterPanel::serialization_key(workspace)
            })
            .ok()
            .flatten()
        {
            Some(serialization_key) => cx
                ...
                .await
                .context("loading writer panel")
                .log_err()
                .flatten()
                .map(|panel| serde_json::from_str::<SerializedWriterPanel>(&panel))
                .transpose()
                .log_err()
                .flatten(),
            None => None,
    ...
    fn new(
        ...
        let writer_panel = cx.new(|cx| {
            ...
            let filter_update_subscription = cx.subscribe_in(
                &filter_editor,
                window,
                |writer_panel: &mut Self, _, event, window, cx| {
                    if let editor::EditorEvent::BufferEdited = event {
                        writer_panel.update_cached_entries(Some(UPDATE_DEBOUNCE), window, cx);
                    }
                },
            );
            ...
            let focus_out_subscription =
                cx.on_focus_out(&focus_handle, window, |writer_panel, _, window, cx| {
                    writer_panel.hide_scrollbar(window, cx);
                });
            let workspace_subscription = cx.subscribe_in(
                ...
                window,
                move |writer_panel, workspace, event, window, cx| {
                    if let workspace::Event::ActiveItemChanged = event {
                        if let Some((new_active_item, new_active_editor)) =
                            workspace_active_editor(workspace.read(cx), cx)
                        {
                            if writer_panel.should_replace_active_item(new_active_item.as_ref()) {
                                writer_panel.replace_active_editor(
                                    new_active_item,
                                    new_active_editor,
                                    window,
                                    cx,
                            ...
                            writer_panel.clear_previous(window, cx);
                            cx.notify();
                        }
                    }
                },
            ...
            let mut writer_panel_settings = *WriterPanelSettings::get_global(cx);
            let mut current_theme = ThemeSettings::get_global(cx).clone();
            let settings_subscription =
                cx.observe_global_in::<SettingsStore>(window, move |writer_panel, window, cx| {
                    let new_settings = WriterPanelSettings::get_global(cx);
                    let new_theme = ThemeSettings::get_global(cx);
                    if &current_theme != new_theme {
                        writer_panel_settings = *new_settings;
                        current_theme = new_theme.clone();
                        for excerpts in writer_panel.excerpts.values_mut() {
                            for excerpt in excerpts.values_mut() {
                                excerpt.invalidate_writers();
                            }
                        }
                        let update_cached_items = writer_panel.update_non_fs_items(window, cx);
                        if update_cached_items {
                            writer_panel.update_cached_entries(Some(UPDATE_DEBOUNCE), window, cx);
                        }
                    } else if &writer_panel_settings != new_settings {
                        writer_panel_settings = *new_settings;
                        cx.notify();
                    }
                });
            ...
            let mut writer_panel = Self {
                ...
                writer_fetch_tasks: HashMap::default(),
            ...
            if let Some((item, editor)) = workspace_active_editor(workspace, cx) {
                writer_panel.replace_active_editor(item, editor, window, cx);
            }
            writer_panel
        });
    ...
    fn serialize(&mut self, cx: &mut Context<Self>) {
        let Some(serialization_key) = self
            .workspace
            .read_with(cx, |workspace, _| WriterPanel::serialization_key(workspace))
            .ok()
            .flatten()
        else {
            return;
        ...
        self.pending_serialization = cx.background_spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        serialization_key,
                        serde_json::to_string(&SerializedWriterPanel { width, active })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
    ...
    fn dispatch_context(&self, window: &mut Window, cx: &mut Context<Self>) -> KeyContext {
        ...
        dispatch_context.add("WriterPanel");
    ...
    fn scroll_editor_to_entry(
        ...
        let scroll_target = match entry {
            ...
            PanelEntry::Outline(OutlineEntry::Outline(writer)) => multi_buffer_snapshot
                .anchor_in_excerpt(writer.excerpt_id, writer.writer.range.start)
                .or_else(|| {
                    multi_buffer_snapshot
                        .anchor_in_excerpt(writer.excerpt_id, writer.writer.range.end)
                }),
    ...
    fn select_parent(&mut self, _: &SelectParent, window: &mut Window, cx: &mut Context<Self>) {
        if let Some(entry_to_select) = self.selected_entry().and_then(|selected_entry| {
            ...
            match &selected_entry {
                ...
                PanelEntry::Outline(OutlineEntry::Outline(writer)) => {
                    previous_entries.find(|entry| {
                        if let PanelEntry::Outline(OutlineEntry::Excerpt(excerpt)) = entry {
                            writer.buffer_id == excerpt.buffer_id && writer.excerpt_id == excerpt.id
                        } else {
                            false
                        }
                    })
    ...
    fn deploy_context_menu(
        ...
        let auto_fold_dirs = WriterPanelSettings::get_global(cx).auto_fold_dirs;
        ...
        let subscription = cx.subscribe(&context_menu, |writer_panel, _, _: &DismissEvent, cx| {
            writer_panel.context_menu.take();
            cx.notify();
        });
    ...
    fn toggle_buffers_fold(
        ...
        cx.spawn_in(window, async move |writer_panel, cx| {
            writer_panel
                .update_in(cx, |writer_panel, window, cx| {
                    active_editor.update(cx, |editor, cx| {
                    ...
                    if let Some(selection) = writer_panel.selected_entry().cloned() {
                        writer_panel.scroll_editor_to_entry(&selection, false, false, window, cx);
                    }
                })
                .ok();
        })
    ...
    fn reveal_entry_for_selection(
        ...
        if !self.active
            || !WriterPanelSettings::get_global(cx).auto_reveal_entries
            || self.focus_handle.contains_focused(window, cx)
        {
            return;
        }
        let project = self.project.clone();
        self.reveal_selection_task = cx.spawn_in(window, async move |writer_panel, cx| {
            cx.background_executor().timer(UPDATE_DEBOUNCE).await;
            let entry_with_selection = writer_panel.update_in(cx, |writer_panel, window, cx| {
                writer_panel.location_for_editor_selection(&editor, window, cx)
            })?;
            let Some(entry_with_selection) = entry_with_selection else {
                writer_panel.update(cx, |writer_panel, cx| {
                    writer_panel.selected_entry = SelectedEntry::None;
                    cx.notify();
                })?;
                return Ok(());
            };
            let related_buffer_entry = match &entry_with_selection {
                ...
                PanelEntry::Outline(writer_entry) => {
                    let (buffer_id, excerpt_id) = writer_entry.ids();
                    writer_panel.update(cx, |writer_panel, cx| {
                        writer_panel
                            .collapsed_entries
                        ...
                        let project = writer_panel.project.read(cx);
                        let entry_id = project
                            .buffer_for_id(buffer_id, cx)
                            .and_then(|buffer| buffer.read(cx).entry_id(cx));
            ...
            writer_panel.update_in(cx, |writer_panel, window, cx| {
                writer_panel.select_entry(entry_with_selection, false, window, cx);
                writer_panel.update_cached_entries(None, window, cx);
            })?;
    ...
    fn render_excerpt(
        ...
        let has_writers = self
            ...
            .and_then(|excerpts| match &excerpts.get(&excerpt.id)?.writers {
                ExcerptOutlines::Outlines(writers) => Some(writers),
                ExcerptOutlines::Invalidated(writers) => Some(writers),
                ExcerptOutlines::NotFetched => None,
            })
            .map_or(false, |writers| !writers.is_empty());
        let is_expanded = !self
            .collapsed_entries
            .contains(&CollapsedEntry::Excerpt(excerpt.buffer_id, excerpt.id));
        let color = entry_label_color(is_active);
        let icon = if has_writers {
    ...
    fn render_writer(
        ...
        let item_id = ElementId::from(SharedString::from(format!(
            "{:?}|{:?}{:?}|{:?}",
            writer.buffer_id, writer.excerpt_id, writer.writer.range, &writer.writer.text,
        )));
        ...
        let is_active = match self.selected_entry() {
            Some(PanelEntry::Outline(OutlineEntry::Outline(selected))) => {
                writer == selected && writer.writer == selected.writer
            }
            _ => false,
        };
        ...
        self.entry_element(
            PanelEntry::Outline(OutlineEntry::Outline(writer.clone())),
            item_id,
            depth,
            icon,
            is_active,
    ...
    fn render_entry(
        ...
        let settings = WriterPanelSettings::get_global(cx);
        let is_active = match self.selected_entry() {
    ...
    fn entry_element(
        ...
        div()
            ...
            .on_click({
                ...
                cx.listener(move |writer_panel, event: &gpui::ClickEvent, window, cx| {
                    if event.down.button == MouseButton::Right || event.down.first_mouse {
                        return;
                    }
                    let change_focus = event.down.click_count > 1;
                    writer_panel.toggle_expanded(&clicked_entry, window, cx);
                    writer_panel.scroll_editor_to_entry(
                        &clicked_entry,
                        true,
                        change_focus,
                        window,
            ...
            .child(
                ListItem::new(item_id)
                    ...
                    .on_secondary_mouse_down(cx.listener(
                        move |writer_panel, event: &MouseDownEvent, window, cx| {
                            // Stop propagation to prevent the catch-all context menu for the project
                            // panel from being deployed.
                            cx.stop_propagation();
                            writer_panel.deploy_context_menu(
                                event.position,
    ...
    fn update_fs_entries(
        ...
        self.fs_entries_update_task = cx.spawn_in(window, async move |writer_panel, cx| {
            ...
            let Ok(buffer_excerpts) = writer_panel.update(cx, |writer_panel, cx| {
                let git_store = writer_panel.project.read(cx).git_store().clone();
                new_collapsed_entries = writer_panel.collapsed_entries.clone();
                new_unfolded_dirs = writer_panel.unfolded_dirs.clone();
                let multi_buffer_snapshot = active_multi_buffer.read(cx).snapshot(cx);
                let buffer_excerpts = multi_buffer_snapshot.excerpts().fold(
                    ...
                    |mut buffer_excerpts, (excerpt_id, buffer_snapshot, excerpt_range)| {
                        ...
                        let is_new = new_entries.contains(&excerpt_id)
                            || !writer_panel.excerpts.contains_key(&buffer_id);
                        ...
                        let writers = match writer_panel
                            .excerpts
                            .get(&buffer_id)
                            .and_then(|excerpts| excerpts.get(&excerpt_id))
                        {
                            Some(old_excerpt) => match &old_excerpt.writers {
                                ExcerptOutlines::Outlines(writers) => {
                                    ExcerptOutlines::Outlines(writers.clone())
                                }
            ...
            writer_panel
                .update_in(cx, |writer_panel, window, cx| {
                    writer_panel.updating_fs_entries = false;
                    writer_panel.new_entries_for_fs_update.clear();
                    writer_panel.excerpts = new_excerpts;
                    writer_panel.collapsed_entries = new_collapsed_entries;
                    writer_panel.unfolded_dirs = new_unfolded_dirs;
                    writer_panel.fs_entries = new_fs_entries;
                    writer_panel.fs_entries_depth = new_depth_map;
                    writer_panel.fs_children_count = new_children_count;
                    writer_panel.update_non_fs_items(window, cx);
                    writer_panel.update_cached_entries(debounce, window, cx);
    ...
    fn replace_active_editor(
        ...
        let buffer_search_subscription = cx.subscribe_in(
            &new_active_editor,
            window,
            |writer_panel: &mut Self,
             _,
             e: &SearchEvent,
             window: &mut Window,
             cx: &mut Context<Self>| {
                if matches!(e, SearchEvent::MatchesInvalidated) {
                    let update_cached_items = writer_panel.update_search_matches(window, cx);
                    if update_cached_items {
                        writer_panel.selected_entry.invalidate();
                        writer_panel.update_cached_entries(Some(UPDATE_DEBOUNCE), window, cx);
                    }
                };
                writer_panel.autoscroll(cx);
            },
        );
    ...
    fn location_for_editor_selection(
        ...
        match &self.mode {
            ...
            ItemsDisplayMode::Outline => self.writer_location(
                buffer_id,
                excerpt_id,
                multi_buffer_snapshot,
                editor_snapshot,
    ...
    fn writer_location(
        ...
        let excerpt_writers = self
            .excerpts
            .get(&buffer_id)
            .and_then(|excerpts| excerpts.get(&excerpt_id))
            .into_iter()
            .flat_map(|excerpt| excerpt.iter_writers())
            .flat_map(|writer| {
                let start = multi_buffer_snapshot
                    .anchor_in_excerpt(excerpt_id, writer.range.start)?
                    .to_display_point(&editor_snapshot);
                let end = multi_buffer_snapshot
                    .anchor_in_excerpt(excerpt_id, writer.range.end)?
                    .to_display_point(&editor_snapshot);
                Some((start..end, writer))
            })
            .collect::<Vec<_>>();

        let mut matching_writer_indices = Vec::new();
        let mut children = HashMap::default();
        let mut parents_stack = Vec::<(&Range<DisplayPoint>, &&Outline, usize)>::new();

        for (i, (writer_range, writer)) in excerpt_writers.iter().enumerate() {
            if writer_range
                .to_inclusive()
                .contains(&selection_display_point)
            {
                matching_writer_indices.push(i);
            } else if (writer_range.start.row()..writer_range.end.row())
                .to_inclusive()
            ...
            while let Some((parent_range, parent_writer, _)) = parents_stack.last() {
                if parent_writer.depth >= writer.depth
                    || !parent_range.contains(&writer_range.start)
                {
                    parents_stack.pop();
                } else {
                    break;
            ...
            parents_stack.push((writer_range, writer, i));
        }

        let writer_item = matching_writer_indices
            .into_iter()
            .flat_map(|i| Some((i, excerpt_writers.get(i)?)))
            .filter(|(i, _)| {
                children
                    .get(i)
                    .map(|children| {
            ...
            .min_by_key(|(_, (writer_range, writer))| {
                let distance_from_start = if writer_range.start > selection_display_point {
                    writer_range.start - selection_display_point
                } else {
                    selection_display_point - writer_range.start
                };
                let distance_from_end = if writer_range.end > selection_display_point {
                    writer_range.end - selection_display_point
                } else {
                    selection_display_point - writer_range.end
                };

                (
                    cmp::Reverse(writer.depth),
                    distance_from_start + distance_from_end,
                )
            })
            .map(|(_, (_, writer))| *writer)
            .cloned();

        let closest_container = match writer_item {
            Some(writer) => PanelEntry::Outline(OutlineEntry::Outline(OutlineEntryOutline {
                buffer_id,
                excerpt_id,
                writer,
            })),
    ...
    fn fetch_outdated_writers(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        for (buffer_id, (buffer_snapshot, excerpt_ranges)) in excerpt_fetch_ranges {
            for (excerpt_id, excerpt_range) in excerpt_ranges {
                ...
                self.writer_fetch_tasks.insert(
                    (buffer_id, excerpt_id),
                    cx.spawn_in(window, async move |writer_panel, cx| {
                        let buffer_language = buffer_snapshot.language().cloned();
                        let fetched_writers = cx
                            .background_spawn(async move {
                                let mut writers = buffer_snapshot
                                ...
                                writers.retain(|writer| {
                                    buffer_language.is_none()
                                        || buffer_language.as_ref()
                                            == buffer_snapshot.language_at(writer.range.start)
                                });
                                writers
                            })
                            .await;
                        writer_panel
                            .update_in(cx, |writer_panel, window, cx| {
                                if let Some(excerpt) = writer_panel
                                    .excerpts
                                    .entry(buffer_id)
                                    .or_default()
                                    .get_mut(&excerpt_id)
                                {
                                    ...
                                    excerpt.writers = ExcerptOutlines::Outlines(fetched_writers);
                                    writer_panel.update_cached_entries(debounce, window, cx);
                                }
                            })
                            .ok();
    ...
    fn invalidate_writers(&mut self, ids: &[ExcerptId]) {
        self.writer_fetch_tasks.clear();
                    ...
                    | FsEntry::ExternalFile(FsEntryExternalFile {
                        ...
                        for &file_excerpt in file_excerpts {
                            if let Some(excerpt) = excerpts
                                ...
                                .filter(|excerpt| excerpt.should_fetch_writers())
                            {
    ...
    fn update_cached_entries(
        ...
        self.cached_entries_update_task = cx.spawn_in(window, async move |writer_panel, cx| {
            if let Some(debounce) = debounce {
                cx.background_executor().timer(debounce).await;
            }
            let Some(new_cached_entries) = writer_panel
                .update_in(cx, |writer_panel, window, cx| {
                    writer_panel.generate_cached_entries(is_singleton, query, window, cx)
                })
                .ok()
            else {
                return;
            ...
            writer_panel
                .update_in(cx, |writer_panel, window, cx| {
                    writer_panel.cached_entries = new_cached_entries;
                    writer_panel.max_width_item_index = max_width_item_index;
                    if writer_panel.selected_entry.is_invalidated()
                        || matches!(writer_panel.selected_entry, SelectedEntry::None)
                    {
                        if let Some(new_selected_entry) =
                            writer_panel.active_editor().and_then(|active_editor| {
                                writer_panel.location_for_editor_selection(
                                    &active_editor,
                                    window,
                                    cx,
                                )
                        ...
                        {
                            writer_panel.select_entry(new_selected_entry, false, window, cx);
                        }
                    }

                    writer_panel.autoscroll(cx);
                    writer_panel.updating_cached_entries = false;
                    cx.notify();
                })
                .ok();
        });
    ...
    fn generate_cached_entries(
        ...
        cx.spawn_in(window, async move |writer_panel, cx| {
            ...
            let Ok(()) = writer_panel.update(cx, |writer_panel, cx| {
                let auto_fold_dirs = WriterPanelSettings::get_global(cx).auto_fold_dirs;
                ...
                for entry in writer_panel.fs_entries.clone() {
                    let is_expanded = writer_panel.is_expanded(&entry);
                        ...
                        ItemsDisplayMode::Outline => {
                            ...
                            if let Some((buffer_id, entry_excerpts)) = excerpts_to_consider {
                                if !active_editor.read(cx).is_buffer_folded(buffer_id, cx) {
                                    writer_panel.add_excerpt_entries(
                                        &mut generation_state,
                                        buffer_id,
                                        entry_excerpts,
                                        depth,
    ...
    fn push_entry(
        ...
        if track_matches {
            ...
            match &entry {
                ...
                PanelEntry::Outline(OutlineEntry::Outline(writer_entry)) => state
                    .match_candidates
                    .push(StringMatchCandidate::new(id, &writer_entry.writer.text)),
    ...
    fn update_non_fs_items(&mut self, window: &mut Window, cx: &mut Context<WriterPanel>) -> bool {
        if !self.active {
            return false;
        }
        ...
        self.fetch_outdated_writers(window, cx);
        if update_cached_items {
            self.selected_entry.invalidate();
        }
        update_cached_items
    ...
    fn add_excerpt_entries(
        ...
        if let Some(excerpts) = self.excerpts.get(&buffer_id) {
            for &excerpt_id in entries_to_add {
                ...
                let mut writer_base_depth = excerpt_depth + 1;
                if is_singleton {
                    writer_base_depth = 0;
                    state.clear();
                ...
                for writer in excerpt.iter_writers() {
                    self.push_entry(
                        state,
                        track_matches,
                        ...
                        writer_base_depth + writer.depth,
                        cx,
                    );
                }
            }
    ...
    fn render_vertical_scrollbar(&self, cx: &mut Context<Self>) -> Option<Stateful<Div>> {
        ...
        Some(
            div()
                ...
                .on_mouse_up(
                    MouseButton::Left,
                    cx.listener(|writer_panel, _, window, cx| {
                        if !writer_panel.vertical_scrollbar_state.is_dragging()
                            && !writer_panel.focus_handle.contains_focused(window, cx)
                        {
                            writer_panel.hide_scrollbar(window, cx);
                            cx.notify();
                        }
        ...
        Scrollbar::horizontal(self.horizontal_scrollbar_state.clone()).map(|scrollbar| {
            div()
                ...
                .on_mouse_up(
                    MouseButton::Left,
                    cx.listener(|writer_panel, _, window, cx| {
                        if !writer_panel.horizontal_scrollbar_state.is_dragging()
                            && !writer_panel.focus_handle.contains_focused(window, cx)
                        {
                            writer_panel.hide_scrollbar(window, cx);
                            cx.notify();
    ...
    fn should_show_scrollbar(cx: &App) -> bool {
        let show = WriterPanelSettings::get_global(cx)
            .scrollbar
            .show
    ...
    fn width_estimate(&self, depth: usize, entry: &PanelEntry, cx: &App) -> u64 {
        let item_text_chars = match entry {
            ...
            PanelEntry::Outline(OutlineEntry::Excerpt(excerpt)) => self
                ...
                .map(|label| label.len())
                .unwrap_or_default(),
            PanelEntry::Outline(OutlineEntry::Outline(entry)) => entry.writer.text.len(),
        ...
        let contents = if self.cached_entries.is_empty() {
            ...
            let list_contents = {
                ...
                uniform_list(
                    "entries",
                    items_len,
                    cx.processor(move |writer_panel, range: Range<usize>, window, cx| {
                        let entries = writer_panel.cached_entries.get(range);
                        entries
                            .map(|entries| entries.to_vec())
                            .unwrap_or_default()
                            .into_iter()
                            .filter_map(|cached_entry| match cached_entry.entry {
                                PanelEntry::Fs(entry) => Some(writer_panel.render_entry(
                                    &entry,
                                    cached_entry.depth,
                                ...
                                PanelEntry::FoldedDirs(folded_dirs_entry) => {
                                    Some(writer_panel.render_folded_dirs(
                                        &folded_dirs_entry,
                                        cached_entry.depth,
                                        cached_entry.string_match.as_ref(),
                                        window,
                                ...
                                PanelEntry::Outline(OutlineEntry::Excerpt(excerpt)) => writer_panel
                                    .render_excerpt(&excerpt, cached_entry.depth, window, cx),
                                PanelEntry::Outline(OutlineEntry::Outline(entry)) => {
                                    Some(writer_panel.render_writer(
                                        &entry,
                                        cached_entry.depth,
                                        cached_entry.string_match.as_ref(),
                                        window,
                                ...
                                PanelEntry::Search(SearchEntry {
                                    ...
                                    kind,
                                    ..
                                }) => writer_panel.render_search_match(
                ...
                .child(
                    div().child(
                        ...
                        .on_click(cx.listener(
                            |writer_panel, _, window, cx| {
                                writer_panel.toggle_active_editor_pin(
                                    &ToggleActiveEditorPin,
                                    window,
                                    cx,
                                );
...
impl Panel for WriterPanel {
    fn persistent_name() -> &'static str {
        "Writer Panel"
    }

    fn position(&self, _: &Window, cx: &App) -> DockPosition {
        match WriterPanelSettings::get_global(cx).dock {
            WriterPanelDockPosition::Left => DockPosition::Left,
            WriterPanelDockPosition::Right => DockPosition::Right,
        }
    }
    ...
    fn set_position(&mut self, position: DockPosition, _: &mut Window, cx: &mut Context<Self>) {
        settings::update_settings_file::<WriterPanelSettings>(
            self.fs.clone(),
            cx,
            move |settings, _| {
                let dock = match position {
                    DockPosition::Left | DockPosition::Bottom => WriterPanelDockPosition::Left,
                    DockPosition::Right => WriterPanelDockPosition::Right,
                };
    ...
    fn size(&self, _: &Window, cx: &App) -> Pixels {
        self.width
            .unwrap_or_else(|| WriterPanelSettings::get_global(cx).default_width)
    }
    ...
    fn icon(&self, _: &Window, cx: &App) -> Option<IconName> {
        WriterPanelSettings::get_global(cx)
            .button
    ...
    fn set_active(&mut self, active: bool, window: &mut Window, cx: &mut Context<Self>) {
        cx.spawn_in(window, async move |writer_panel, cx| {
            writer_panel
                .update_in(cx, |writer_panel, window, cx| {
                    let old_active = writer_panel.active;
                    writer_panel.active = active;
                    if old_active != active {
                        if active {
                            if let Some((active_item, active_editor)) =
                                writer_panel.workspace.upgrade().and_then(|workspace| {
                                    workspace_active_editor(workspace.read(cx), cx)
                                })
                            {
                                if writer_panel.should_replace_active_item(active_item.as_ref()) {
                                    writer_panel.replace_active_editor(
                                        active_item,
                                        active_editor,
                                        window,
                                    ...
                                    writer_panel.update_fs_entries(active_editor, None, window, cx)
                                }
                                return;
                            }
                        }

                        if !writer_panel.pinned {
                            writer_panel.clear_previous(window, cx);
                        }
                    }
                    writer_panel.serialize(cx);
                })
                .ok();
        })
        .detach()
...
impl Focusable for WriterPanel {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        self.filter_editor.focus_handle(cx).clone()
    }
}

impl EventEmitter<Event> for WriterPanel {}

impl EventEmitter<PanelEvent> for WriterPanel {}

impl Render for WriterPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_mouse_down(
                MouseButton::Right,
                cx.listener(move |writer_panel, event: &MouseDownEvent, window, cx| {
                    if let Some(entry) = writer_panel.selected_entry().cloned() {
                        writer_panel.deploy_context_menu(event.position, entry, window, cx)
                    } else if let Some(entry) = writer_panel.fs_entries.first().cloned() {
                        writer_panel.deploy_context_menu(
                            event.position,
                            PanelEntry::Fs(entry),
                            window,
            ...
            .when_some(search_query, |writer_panel, search_state| {
                writer_panel.child(
                    h_flex()
                        .py_1p5()
                        .px_2()
...
fn find_active_indent_guide_ix(
    writer_panel: &WriterPanel,
    candidates: &[IndentGuideLayout],
) -> Option<usize> {
    let SelectedEntry::Valid(_, target_ix) = &writer_panel.selected_entry else {
        return None;
    };
    let target_depth = writer_panel
        .cached_entries
        .get(*target_ix)
        .map(|cached_entry| cached_entry.depth)?;

    let (target_ix, target_depth) = if let Some(target_depth) = writer_panel
        .cached_entries
        .get(target_ix + 1)
...
fn subscribe_for_editor_events(
    ...
    cx.subscribe_in(
        editor,
        window,
        move |writer_panel, editor, e: &EditorEvent, window, cx| {
            if !writer_panel.active {
                return;
            }
            match e {
                EditorEvent::SelectionsChanged { local: true } => {
                    writer_panel.reveal_entry_for_selection(editor.clone(), window, cx);
                    cx.notify();
                }
                EditorEvent::ExcerptsAdded { excerpts, .. } => {
                    ...
                    writer_panel.update_fs_entries(editor.clone(), debounce, window, cx);
                }
                ...
                EditorEvent::ExcerptsExpanded { ids } => {
                    writer_panel.invalidate_writers(ids);
                    let update_cached_items = writer_panel.update_non_fs_items(window, cx);
                    if update_cached_items {
                        writer_panel.update_cached_entries(Some(UPDATE_DEBOUNCE), window, cx);
                    }
                ...
                EditorEvent::BufferFoldToggled { ids, .. } => {
                    ...
                    writer_panel.new_entries_for_fs_update.extend(
                        ids.iter()
                            .filter(|id| {
                                writer_panel
                                    .excerpts
                                    ...
                                    .find_map(|(buffer_id, excerpts)| {
                                        if excerpts.contains_key(id) {
                                            ignore_selections_change |= writer_panel
                                                .preserve_selection_on_buffer_fold_toggles
                                                .remove(buffer_id);
                                            Some(buffer_id)
                                        } else {
                    ...
                    if !ignore_selections_change {
                        if let Some(entry_to_select) = latest_unfolded_buffer_id
                            ...
                            .map(PanelEntry::Fs)
                        {
                            writer_panel.select_entry(entry_to_select, true, window, cx);
                        }
                    }
                ...
                EditorEvent::Reparsed(buffer_id) => {
                    if let Some(excerpts) = writer_panel.excerpts.get_mut(buffer_id) {
...
mod tests {
    ...
    async fn test_project_search_results_toggling(cx: &mut TestAppContext) {
        ...
        let workspace = add_writer_panel(&project, cx).await;
        let cx = &mut VisualTestContext::from_window(*workspace, cx);
        let writer_panel = writer_panel(&workspace, cx);
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.set_active(true, window, cx)
        });
      ...
      config.rs
        ...
        writer_panel.update(cx, |writer_panel, cx| {
            assert_eq!(
                display_entries(
                    &project,
                    &snapshot(&writer_panel, cx),
                    &writer_panel.cached_entries,
                    writer_panel.selected_entry(),
                    cx,
                ),
        ...
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.select_parent(&SelectParent, window, cx);
            assert_eq!(
                display_entries(
                    &project,
                    &snapshot(&writer_panel, cx),
        ...
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.collapse_selected_entry(&CollapseSelectedEntry, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.expand_all_entries(&ExpandAllEntries, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.expand_selected_entry(&ExpandSelectedEntry, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        let filter_text = "a";
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.filter_editor.update(cx, |filter_editor, cx| {
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        let active_editor = writer_panel.read_with(cx, |writer_panel, _| {
            writer_panel
                .active_editor()
                .expect("should have an active editor open")
        });
        let initial_writer_selection =
            "search: match config.param_names_for_lifetime_elision_hints {";
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            assert_eq!(
                display_entries(
                ...
                select_first_in_all_matches(initial_writer_selection)
            );
            assert_eq!(
                selected_row_text(&active_editor, cx),
                initial_writer_selection.replace("search: ", ""), // Clear writer metadata prefixes
                "Should place the initial editor selection on the corresponding search result"
            );

            writer_panel.select_next(&SelectNext, window, cx);
            writer_panel.select_next(&SelectNext, window, cx);
        });

        let navigated_writer_selection =
            "search: Some(it) if config.param_names_for_lifetime_elision_hints => {";
        writer_panel.update(cx, |writer_panel, cx| {
            assert_eq!(
                display_entries(
                ...
                select_first_in_all_matches(navigated_writer_selection)
            );
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        writer_panel.update(cx, |_, cx| {
            assert_eq!(
                selected_row_text(&active_editor, cx),
                navigated_writer_selection.replace("search: ", ""), // Clear writer metadata prefixes
                "Should still have the initial caret position after SelectNext calls"
            );
        });

        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.open_selected_entry(&OpenSelectedEntry, window, cx);
        });
        writer_panel.update(cx, |_writer_panel, cx| {
            assert_eq!(
                selected_row_text(&active_editor, cx),
                navigated_writer_selection.replace("search: ", ""), // Clear writer metadata prefixes
                "After opening, should move the caret to the opened writer entry's position"
            );
        });
        ...
        let next_navigated_writer_selection = "search: InlayHintsConfig { param_names_for_lifetime_elision_hints: true, ..TEST_CONFIG },";
        writer_panel.update(cx, |writer_panel, cx| {
            assert_eq!(
                display_entries(
                    &project,
                ...
                select_first_in_all_matches(next_navigated_writer_selection)
            );
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        writer_panel.update(cx, |_writer_panel, cx| {
            assert_eq!(
                selected_row_text(&active_editor, cx),
                next_navigated_writer_selection.replace("search: ", ""), // Clear writer metadata prefixes
                "Should again preserve the selection after another SelectNext call"
            );
        });

        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.open_excerpts(&editor::OpenExcerpts, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
        let new_active_editor = writer_panel.read_with(cx, |writer_panel, _| {
            writer_panel
                .active_editor()
                .expect("should have an active editor open")
        });
        writer_panel.update(cx, |writer_panel, cx| {
            ...
            assert_eq!(
                ...
                next_navigated_writer_selection.replace("search: ", ""), // Clear writer metadata prefixes
                "When opening the excerpt, should navigate to the place corresponding the writer entry"
            );
        });
    }
...
/root/two/
  b.txt
    search: a aaa"#
        ...
        writer_panel.update_in(cx, |writer_panel, window, cx| {
            writer_panel.select_previous(&SelectPrevious, window, cx);
            writer_panel.open_selected_entry(&OpenSelectedEntry, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
...
"#,
        ...
        writer_panel.update(cx, |writer_panel, cx| {
            assert_eq!(
                ...
                indoc!(
                    "
writer: struct OutlineEntryExcerpt
writer: id
writer: buffer_id
writer: range"
                )
        ...
        writer_panel.update(cx, |writer_panel, cx| {
            assert_eq!(
                ...
                indoc!(
                    "
writer: struct OutlineEntryExcerpt  <==== selected
writer: id
writer: buffer_id
writer: range"
                )
...
writer: range  <==== selected"
  ...
  src/
    ...
    async fn add_writer_panel(
        project: &Entity<Project>,
        cx: &mut TestAppContext,
    ) -> WindowHandle<Workspace> {
        let window = cx.add_window(|window, cx| Workspace::test_new(project.clone(), window, cx));

        let writer_panel = window
            .update(cx, |_, window, cx| {
                cx.spawn_in(window, async |this, cx| {
                    WriterPanel::load(this, cx.clone()).await
                })
            })
            .unwrap()
            .await
            .expect("Failed to load writer panel");

        window
            .update(cx, |workspace, window, cx| {
                workspace.add_panel(writer_panel, window, cx);
            })
            .unwrap();
        window
    }
    ...
    fn writer_panel(
        ...
        workspace
            .update(cx, |workspace, _, cx| {
                workspace.panel::<WriterPanel>(cx).expect("no writer panel")
            })
            .unwrap()
    }
        ...
        for entry in cached_entries {
            ...
            display_string += &match &entry.entry {
                ...
                PanelEntry::FoldedDirs(folded_dirs) => folded_dirs
                    ...
                    .map(|name| name.to_string_lossy().to_string() + "/")
                    .collect(),
                PanelEntry::Outline(writer_entry) => match writer_entry {
                    OutlineEntry::Excerpt(_) => continue,
                    OutlineEntry::Outline(writer_entry) => {
                        format!("writer: {}", writer_entry.writer.text)
                    }
                },
...
// ... snip
    ...
    fn snapshot(writer_panel: &WriterPanel, cx: &App) -> MultiBufferSnapshot {
        writer_panel
            .active_editor()
            .unwrap()
            .read(cx)
    ...
    fn selected_row_text(editor: &Entity<Editor>, cx: &mut App) -> String {
        editor.update(cx, |editor, cx| {
                ...
                assert_eq!(selections.len(), 1, "Active editor should have exactly one selection after any writer panel interactions");
